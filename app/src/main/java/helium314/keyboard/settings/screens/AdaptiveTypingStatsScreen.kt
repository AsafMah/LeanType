...
private fun MockKeyboardHeatmap(stats: List<TouchModelDao.Stat>) {
    ...
    // Key by the LOWER-CASE code and keep the highest-confidence stat per physical key, so taps
    // recorded under a shifted (upper-case) code still render on the lower-case heat-map cells.
    val byCode = (if (pref.isNotEmpty()) pref else stats)
        .groupBy { Character.toLowerCase(it.keyCode) }
        .mapValues { (_, list) -> list.maxByOrNull { it.count }!! }
