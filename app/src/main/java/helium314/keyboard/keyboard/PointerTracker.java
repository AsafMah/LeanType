...
public final class PointerTracker implements PointerTrackerQueue.Element,
    ...
    private void recordAdaptiveTouchSample(final Key key, final int x, final int y) {
        ...
        // Measure the landing offset in the SAME coordinate space the hit box lives in: detection
        // applies the key-detector correction (getTouchX/Y), so we must too, otherwise a non-zero
        // keyboard-origin correction would skew every learned offset by a constant. The adaptive
        // bias never moves these coordinates, so the model trains on the raw finger position (no
        // learn-on-corrected-output feedback loop).
        final float dx = mKeyDetector.getTouchX(x) - hitBox.exactCenterX();
        final float dy = mKeyDetector.getTouchY(y) - hitBox.exactCenterY();
